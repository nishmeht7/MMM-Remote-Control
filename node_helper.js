...
module.exports = NodeHelper.create(Object.assign({
        ...
        answerPost: function(query, req, res) {
            ...
            if (query.data === "config") {
                ...
                source.on("end", () => {
                	var config = Object.assign({}, self.configOnHd, req.body);
                    self.configOnHd = self.removeDefaultValues(config);
            ...
            if (query.data === "saveTemplate") {
                var configPath = path.resolve("config/saved/config"+Date.now()+".js");

				var config = self.removeDefaultValues(req.body);

				var header = "/*************** AUTO GENERATED BY REMOTE CONTROL MODULE ***************/\n\nvar config = \n";
				var footer = "\n\n/*************** DO NOT EDIT THE LINE BELOW ***************/\nif (typeof module !== 'undefined') {module.exports = config;}\n";

				fs.writeFile(configPath, header + util.inspect(config, {
						showHidden: false,
						depth: null,
						maxArrayLength: null,
						compact: false
					...
					(error) => {
						...
						if (error) {
							self.sendResponse(res, error, { query: query, config: config });
						}
						console.info("MMM-Remote-Control saved new template!");
						self.sendResponse(res, undefined, { query: query, config: config });
					}
				);
            }

            if (query.data === "deleteTemplate") {
				try {
					fs.unlinkSync("config/saved/"+req.body);
					self.loadConfigList();
				} catch (error) {
					self.sendResponse(res, error, { query: query });
				}
				console.info("MMM-Remote-Control deleted template!");
				self.sendResponse(res, undefined, { query: query });
            }
        },
